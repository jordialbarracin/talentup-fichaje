...
async def close_payroll(
    ...
    """
    Calculate and close payroll for a month.
    For each active employee: calculate hours, overtime, and generate payroll record.
    """
    ...
    # Get all clock_ins for the month
    ...
    # Get all overtime for the month
    ot_query = select(Overtime).where(
    ...
    for emp in employees:
        ...
        for day_date, day_clock in daily_clock.items():
            ...
            if first_in and last_out:
                ...
                worked_min += int(worked_seconds / 60)
        ...
        # Gross total (simplified)
        gross = base_salary + ot_amount
        ss_ded = gross * 0.0635  # 6.35% SS
        irpf = gross * 0.12  # 12% IRPF (simplified)
    ...
    await log_action(db, tenant_id=tenant_id, user_id=current_user.id,
                     action="close_payroll", entity_type="payroll",
    ...
    return {
        ...
        "year": year,
        "period_label": period_label,
        "employees_processed": len(created),
        "payrolls": [p.to_dict() for p in created],
    }
